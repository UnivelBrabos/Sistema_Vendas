...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _loadData() async {
    try {
      final vendeResp = await supabase
          .from('vendedores')
          .select('id_vendedor, nome');
      for (final row in (vendeResp as List).cast<Map<String, dynamic>>()) {
        final id = (row['id_vendedor'] as num).toString();
        _vendorNames[id] = row['nome'] as String;
      }

      final vendasResp = await supabase
          .from('vendas')
          .select('id_vendedor, id_cliente, total')
          .order('data_venda', ascending: false);
      _allVendas = (vendasResp as List).cast<Map<String, dynamic>>();
  ...
  void _applyFilter() {
    final filtered = _selectedVendorId == null
        ? _allVendas
        : _allVendas
            .where((v) => (v['id_vendedor'] as num).toString() == _selectedVendorId)
            .toList();
    ...
    for (var v in filtered) {
      ...
      gastosPorCliente[cid] =
          (gastosPorCliente[cid] ?? 0) + (v['total'] as num).toDouble();
    }
    ...
    for (var v in filtered) {
      ...
      byVendor[vid] =
          (byVendor[vid] ?? 0) + (v['total'] as num).toDouble();
    }
